...
A \textbf{inferência lógica} é o processo de se tirar conclusões dadas premissas e regras previamente estabelecidas. Existem algumas maneiras de se trabalhar esse processo de pensamento. Para a construção das regras de inferência da lógica proposicional começa com sua axiomatização:

\begin{axioms}[Frege-Lukasiewicz]
Sejam p, q e r proposições: \\
\textbf{FL1)} $p \rightarrow (q \rightarrow p)$ \\
\textbf{FL2)} $p \rightarrow (q \rightarrow r) \rightarrow (p \rightarrow  q \rightarrow [p \rightarrow  r])  $ \\
\textbf{FL3)} $\sim p \rightarrow  \sim q \rightarrow  (q \rightarrow p)$
\end{axioms}

Os axiomas podem ser um pouco confusos e pouco intuitivos. Para que não tenhamos que lidar apenas com eles durante o processo de inferência utilizamos as \textbf{regras de inferência}.
\\~\\
Antes de falarmos sobre elas vamos formalizar o conceito de inferência lógica:

\begin{definition}[Inferência Lógica]
Sejam $p_0, p_1, ..., p_n$ e $q$ proposições, se $q$ é uma consequência lógica de $p_0, p_1, ..., p_n$, então $p_0, p_1, ..., p_n$ inferem em $q$, denotamos por:
$$p_0, p_1, ..., p_n \Rightarrow q$$
\end{definition}

\break

\begin{definition}[Regras de Inferência]
Sejam P, Q, R e S fórmulas proposicionais:
\\~\\
\textbf{Modus Ponens:}
...
\textbf{Dilemas Construtivo:}
$$(P \rightarrow Q) \land (R \rightarrow S), P \lor R \Rightarrow Q \lor S $$

\textbf{Dilema Destrutivo:}
$$(P \rightarrow Q) \land (R \rightarrow S), \sim P \lor \sim R \Rightarrow Q \lor S $$
...
Apesar de definidas em uma linha não é dessa forma que descrevemos um processo de inferência, seria algo mais semelhando à uma tabela:
\begin{align*}
Modus \ Ponens && Modus \ Tollens && Elimina\c{c}\~{a}o \\
P \implies Q & & P \implies Q     & & P \lor Q \\
P            & & \sim Q           & & \sim P   \\
\dfrac{\hspace*{50pt}}{\therefore Q}  & & \dfrac{\hspace*{50pt}}{\therefore \sim P} & & \dfrac{\hspace*{50pt}}{\therefore Q}
\end{align*}

Os diagramas acima são fáceis de se ler. Tudo que está em cima da linha é dado como verdadeiro. O que está abaixo é a consequência lógica do que está acima.
\\~\\
Do processo de inferência podemos gerar o conceito de \textbf{prova formal}:
\begin{definition}[Prova Formal]
Uma prova formal da fórmula $q$ (conclusão) a partir de $p_0, ..., p_n$ é uma sequência de fórmulas:
$$p_0, p_1,..., p_n, q_0, ..., q_m$$
De tal forma que $q_m = q$, e para todo $i$ $q_i$ é um axioma se $i=0$ então $p_0, ..., p_n \Rightarrow q_0$, caso $i > 0$ então $p_0, ..., p_n, q_0, ..., q_{i-1} \Rightarrow q_i$.
\\~\\
Se existe prova formal de $q$ partindo de $p_0, ..., p_n$ então $q$ é \textbf{deduzido} de $p_0, ..., p_n$ e denotamos isso por:
$$p_0, ..., p_n \vdash q$$
Caso não hajam premissas $q$ é consequência do axioma $q_0$ e das conclusões posteriores $q_1, ..., q_{m}$ uma prova formar é a sequência:
$$q_0, q_1 ..., q_{m-1}, q_m$$
Como $q = 1_m$, se $i = 0$, $q_i$ é um axioma, se $i > 0$, tal que:
$$q_0, ..., q_{i-1} \Rightarrow q_i$$
Escreva $\vdash q$ e chame $q$ de \textbf{teorema}
\end{definition}

Apesar do formalismo deixar essa definição um pouco confusa ela é muito simples. Veja:
Há um grupo de premissas $p$ e um de consequência $q$. Se $q_i$ onde $i=0$ então $q_i$ é um axioma por ser autoevidente e deduzido diretamente das premissas.
\\~\\
Um exemplo disso é a função sucessor $S(n)$ \footnote{Para todo numero natural $n$ existe um sucessor $n+1$ e se $S(n) = S(m)$, então $m=n$\\ Não se apegue muito à isso, veremos com mais rigor adiante} dos \textbf{Axiomas de Peano}. Para isso assumimos como premissa que todo número natural tem um sucessor e então deduzimos os axiomas.
\\~\\
Se $q_i$ e $i > 0$ então $q_i$ e não temos premissas, então $q_i$ é um teorema por basear-se nos axiomas e em teoremas do qual depende. Continuando com o exemplo dos axiomas de peano temos o teorema:

\begin{theorem}
$2 \neq 4$
\end{theorem}
A prova disso não precisa tomar nenhuma premissa, apenas os axiomas que temos conhecimento.\\
\\~\\
\textit{Prova:} Assuma que $2 = 4$, então $S(2) = S(4)$, porém sabemos que $S(2) = 3$ e $S(3) = 4$, logo, $3 = 4$, o que é um absurdo. $\blacksquare$
\\~\\
O leitor não precisa memorizar todos as regras de inferência, mas sim compreender como a lógica torna-se o fundamento da matemática por meio dessas operações, e como os processos só são válidos por obedecerem as regras da lógica. É importante que você internalize esses métodos de pensamento. Você usará bastante ao longo da sua jornada pela matemática.
